...
# Логгирование
import logging
logging.basicConfig(format = u'%(filename)s[LINE:%(lineno)d]# %(levelname)-8s [%(asctime)s]  %(message)s', level = logging.INFO, filename = directory + 'cipher.log')
...
def main(message):
    ...
    if step == 'main':
        ...
        if text == '**мои карты**':
            ...
            sent = bot.send_message(mid, 'Выберите карту (всего карт ' + str(len(sessionStorage[mid]['cards'])) + ')', reply_markup = keybGR)
        ...
        for i, elem in enumerate(cards):
            if text in elem:
                single_msg = types.InlineQueryResultArticle(
                    id=str(i), title=elem,
                    input_message_content=types.InputTextMessageContent(message_text='Карта *' + elem + '*\nНомер: `' + str(sessionStorage[uid]['cards'][i]['num']) + '`', parse_mode = 'Markdown'),
                    reply_markup=None
                )
